...
DOC_PROJECT_REPLACEMENTS = [
    (r'"NAME: SPEC; NAME: SPEC; ..."', r'``"NAME: SPEC; NAME: SPEC; …"``'),
    (
    ...
    (re.compile(r'(?<!`)index\.html'), r'``index.html``'),
    (re.compile(r'(?<!`)versions\.json'), r'``versions.json``'),
...
def _get_help_record(opt, ctx):
    ...
    if _get_envvar_name(opt, ctx):
        bracket_info.append("env var ``%s``" % _get_envvar_name(opt, ctx))
    ...
    if opt.default is not None and opt.show_default:
        ...
        bracket_info.append('default: ``%s``' % default_val)

    if bracket_info:
        out.append('[' + "; ".join(bracket_info) + ']')
...
def _format_option(opt, ctx):
    """Format the output for a `click.Option`."""
    opt = _get_help_record(opt, ctx)
...
def _format_envvar(param, ctx):
    """Format the envvars of a `click.Option` or `click.Argument`."""
    yield '.. envvar:: {}'.format(_get_envvar_name(param, ctx))
    yield '   :noindex:'
    yield ''
...
def _get_envvar_name(param, ctx):
    if not param.show_envvar:
        return None
    envvar = param.envvar
    if envvar is None:
        if param.allow_from_autoenv and ctx.auto_envvar_prefix is not None:
            envvar = "{}_{}".format(ctx.auto_envvar_prefix, param.name.upper())
    return envvar


def _format_envvars(ctx):
    """Format all envvars for a `click.Command`."""
    params = [x for x in ctx.command.params if _get_envvar_name(x, ctx)]

    for param in params:
        yield '.. _{command_name}-{param_name}-{envvar}:'.format(
            command_name=ctx.command_path.replace(' ', '-'),
            param_name=param.name,
            envvar=_get_envvar_name(param, ctx),
        )
        yield ''
        for line in _format_envvar(param, ctx):
            yield line
        yield ''
...
def _format_command(ctx, nested, commands=None):
    ...
    # yield _do_project_replacements(line)
